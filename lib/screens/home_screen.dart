  ...
  void _showAddMusicDialog() {
    showModalBottomSheet(
      ...
      builder: (context) => AddMusicBottomSheet(
        onAdd: (title, artist, url) async {
          if (!_musicService.isValidMusicUrl(url)) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Only YouTube, Spotify, TikTok or SoundCloud links are supported'),
          ...
          final source = MusicModel.detectSource(url);
          final videoId = source == MusicSource.youtube
              ? MusicModel.extractYouTubeId(url)
              : null;

          final id = await _musicService.addMusic(
            userId: currentUser.uid,
            username: _currentUser!.username,
            userPhotoUrl: _currentUser!.photoUrl,
            userBadge: _currentUser!.badge,
            title: title,
            artist: artist,
            musicUrl: url,
            source: source.name,
          ...
          if (id != null) {
            ...
            await _loadUserAndMusic();
          } else {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Failed to add music, please try again'),
                backgroundColor: Colors.red,
              ),
            );
          }
