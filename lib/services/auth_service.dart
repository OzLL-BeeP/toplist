...
class AuthService {
  ...
  // IMPORTANT: This should only ever be called from a trusted context
  // (e.g. a verified payment webhook / Cloud Function), never directly
  // from client-side UI in response to unverified user input.
  Future<void> updateUserSubscription({
    ...
    try {
      await _firestore.collection('users').doc(uid).update({
        ...
        'subscription.startedAt': DateTime.now().toIso8601String(),
        'subscription.expiresAt': expiresAt?.toIso8601String(),
      });
    } catch (e) {
